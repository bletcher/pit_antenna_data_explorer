...
# read down through the cols, ops, vals variables for filter conditions
cols_bkt <- list("cohort",  "species", "river")
ops_bkt <-  list("%in%",    "==", "%in%")
vals_bkt <- list(2002:2015, "bkt", c("west brook", "wb jimmy", "wb mitchell", "wb obear"))

######################################
# Loop over cohorts and rivers
dataCMR_bkt_target <- tar_plan(
  tar_map(
    values = list(
    ...
    tar_target(
      name = paste0("eh_bkt_", river, "_", cohort, "_target"),
      command = getEH_AIS(
        cdWB_CMR0_target2, 
        cols_bkt, 
        ops_bkt, 
        list(cohort, "bkt", paste0("wb ", river)),
        maxAgeInSamples2
      )
    )
  )
